...
public class MainSignUp extends AppCompatActivity {
    ...
    public void onClickSubmitSignUp(View a) {
        ...
        if (isEmpty()) {
            ...
            // toast -> empty fields
            Toast.makeText(MainSignUp.this, "Empty fields detected!", Toast.LENGTH_SHORT).show();

        } else if (isInvalidAmount()) {

            // toast -> not minimum amount
            Toast.makeText(MainSignUp.this, "Enter a valid initial deposit!", Toast.LENGTH_SHORT).show();
    ...
    public void submitSignUpValues() {
        ...
        cvBankAccount.put(BankAccount.KEY_BANKACCOUNT_PIN, SecurityManager.encryptIt(editTextPinCode.getText().toString().trim()));
        ...
        objPersonBankAccount.setBankAccountPin(SecurityManager.encryptIt(editTextPinCode.getText().toString().trim()));
    ...
    public boolean isInvalidAmount() {
        ...
        // validate current editText if contains minimum deposit amounts
        boolIsNotMinimum = Validation.validateAmount(editTextIDeposit);
